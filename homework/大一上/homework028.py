def main():
    ...
    toltoInfectProple += first_confirmed #新增總確診人數
    for i in range(2,day+1):
        nowInfect = int(((infect / rehabilitation) * (1-immunity))*data[i-1][0]) #計算今天確診人數
        if toltoInfectProple+startRehabilitationPeople+nowInfect > people: #如果超過人數，就全部剪掉剩下
            nowInfect = people-toltoInfectProple-startRehabilitationPeople
        ...
        toltoInfectProple += data[i][1] #儲存總確診人數
    
    for i in range(1,day+1):
        print(f"{i} {data[i][0]:.0f} {data[i][1]:.0f} {data[i][2]:.0f}")
    print(int(toltoInfectProple))
main()