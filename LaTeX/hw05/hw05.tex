...
\end{shaded}  
\begin{enumerate}[label=(\alph*)]
	\item Plugging in $n = \answer{5.1.18a}{2}$, we see that $P(2)$ is the statement $2! < 2^2$.
	\item Since $2! = \answer{5.1.18a}{2}$, this is the true statement $2 < 4$.
	\item The inductive hypothesis is the statement that \answer{5.1.18c}{$k!<k^k$}.
    \item For the inductive step, we want to show for each $k \ge 2$ that $P(k)$ implies \answer{5.1.18d}{$P(k+1)$}. In other words, we want to show that assuming the inductive hypothesis (see part (c)) we can prove that \answer{5.1.18e}{$(k+1)!<(k+1)^{k+1}$}.
    \item $(k+1)! = \answer{5.1.18f}{(k+1)\cdot k!} < \answer{5.1.18g}{(k+1)\cdot k^k} < \answer{5.1.18h}{(k+1)\cdot (k+1)^k} = (k + 1)^{k+1}$
